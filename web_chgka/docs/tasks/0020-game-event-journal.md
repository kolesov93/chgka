...
- Формы входа admin-entrypoints подписаны `[ведущий]` и `[история игр]`; document title равен «Что? Где? Когда? — Ведущий» и «Что? Где? Когда? — История игр». Player-entrypoint остаётся без подзаголовка и с базовым title.
...
3. Перейти на `/admin`: форма входа должна показывать `[ведущий]`, вкладка браузера — «Что? Где? Когда? — Ведущий». После входа в комнате ожидания должен быть виден режим текущей игры с default «Тестовая». Переключить на «Обычная», обновить `/admin` и убедиться, что выбор сохранился.
4. Открыть рядом `/play`: под «Что? Где? Когда?» не должно быть подзаголовка, title вкладки остаётся базовым «Что? Где? Когда?». Войти игроком и начать игру. Блок режима должен остаться видимым в live-панели ведущего во всех фазах; самой истории на `/admin` быть не должно.
