...
1. Запустить development Compose, открыть `/admin/history`. Без admin token должна быть только форма пароля; после входа — только экран «История игр», без стола, игроков, игровых кнопок и отдельного блока режима текущей игры. Сам вход в историю не должен добавлять пустую сессию.
