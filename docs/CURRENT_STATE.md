...
- Active task: none.
- Branch: `main`
- Status: task 0032 is merged and published. The deployment runbook now uses the current `main` release branch; no runtime behavior changed and no manual smoke is required.

## Repository checkpoint

- Task 0032 implementation commit is `803d1d8`, task merge is `e549922`, and remote-integration merge is `beaf31f`. It corrects the new-release runbook to require a committed `main` revision with green GitHub CI and to run `git switch main` before pulling and archiving. The stale branch-specific wording is gone; this is documentation-only and requires no browser smoke.
