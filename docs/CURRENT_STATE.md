...
- Active task: none.
- Branch: `main`
- Status: task 0031 is complete and published at the branch-history level. GitHub `main` contains the redacted history; the private hostname is absent from its current tree, all reachable historical snapshots, and commit messages. GitHub still returns the now-unreachable old commit objects by a known SHA, so literal server-side purge remains a GitHub Support operation. The legacy tag is unchanged and the VPS was not accessed.
...
- The accepted state transitions, timer segmentation, race/reconnect/Live Ops handling, typed journal events and verification summary are recorded in `docs/tasks/0023-earned-minutes-credit.md`.
...
- Task 0023 closure commit: `5d86869` (`Close earned minutes and credit task`).
- Task 0023 merge commit: `0e36d43` (`Merge earned minutes and credit task`).
...
During task 0029, untracked legacy files were moved without deletion to `/home/kolesov93/Programming/chgka2-legacy-local-files-20260813/`. The old Docker-owned `web_chgka/runtime-data` remains locally until its ownership/move is resolved; it is ignored and is not part of Git. Ignored root `runtime-data`, `frontend/node_modules`, `frontend/dist`, and Python/pytest caches are local artifacts. `frontend/package-lock.json` is the committed source of truth for reproducible frontend installs.
