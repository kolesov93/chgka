...
- Runtime UI does not use browser-native `confirm`, `alert`, or `prompt`. Ordinary actions emit immediately; early publication of answer-section media uses a token-bound inline confirmation in the admin media panel, while a kicked player receives the server explanation through the persistent login `sessionNotice`. The captain-request modal remains application-owned game UI.
...
- `frontend/src/appPaths.js` normalizes the Vite base and is the single source for entrypoints plus static image/sound URLs. `frontend/src/backendUrls.js` builds the corresponding media/intro and Socket.IO transport paths. Development still connects directly to root routes on `http://localhost:8000`; production uses the current origin under the configured base (`/` by default, `/chgka/` for the temporary public path).
